		...
		<div class="block_content">
			...
			{foreach from=$products item=product name=homeFeaturedProducts}
				...
				<li class="ajax_block_product {if $smarty.foreach.homeFeaturedProducts.first}first_item{elseif $smarty.foreach.homeFeaturedProducts.last}last_item{else}item{/if} {if $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 1} {/if} {if $smarty.foreach.homeFeaturedProducts.iteration > ($smarty.foreach.homeFeaturedProducts.total - $totModulo)}last_line{/if}">
					...
					<p class="s_title_block"><a href="{$product.link}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'}</a></p>
...
<!-- 						<span class="price">{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span> -->
					{*Discount patch *}
						...
						{if !$priceDisplay}{$newPrice = $product.price-$quantity_discount.reduction}{else}{$newPrice = $product.price_tax_exc-$quantity_discount.reduction}{/if}
						...
						<span class="price" style="display: inline;">
							<span style="font-size:14px;">{convertPrice price=$low|floatval} - {if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span>
						</span>
						{else}
						...
						{/if}
					{*Discount patch *}
						</p>{else}<div style="height:21px;"></div>{/if}
