...
<!-- 		<p class="title_block">{l s='List of pages in %s:' sprintf=$cms_category->name}</p> -->
			<ul class="bullet">
				{foreach from=$cms_pages item=cmspages}
					<li>
						<a href="{$link->getCMSLink($cmspages.id_cms, $cmspages.link_rewrite)|escape:'htmlall':'UTF-8'}" style="font-size:17px;">{$cmspages.meta_title|escape:'htmlall':'UTF-8'}<br></a>
						{if $cmspages.short_description}
						<div style="border:1px solid #b7b7b7; background-color:#e0e0e0; margin-left:20px; font-size:12px; font-family: arial,helvetica,sans-serif;">{$cmspages.short_description|nl2br}</div>
						<a href="{$link->getCMSLink($cmspages.id_cms, $cmspages.link_rewrite)|escape:'htmlall':'UTF-8'}" style="font-size:13px;margin-bottom:20px;display:block;margin-left:20px">Läs mer..<br></a>
						{/if}
					</li>
				{/foreach}
			</ul>
