...
<form method="post"> 
	<table id="cart_summary" class="std">
		...
		<tfoot>
			...
			<tr>
				<td colspan="5">&nbsp;</td>
				<td colspan="2" class="update_cart" style="border-right:1px solid #999;">
						<input type="submit" name="submitUpdateCart" value="{l s='Uppdatera varukorgen'}" class="button"/>
				</td>
			</tr>
			{/if}
		{if $use_taxes}
				...
				{if $voucherAllowed}
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
						<fieldset>
							...
							<span class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='OK'}" class="button" /></span>
						</fieldset>
					</form>
					{if $displayVouchers}
...
<p>
	...
	<label for="allow_seperated_package" id="allow_seperated_package_label">{l s='Send the available products first'}</label>
</p>
{/if}
{if !$opc}
