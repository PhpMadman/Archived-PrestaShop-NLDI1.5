...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		{if $have_image}
			...
			{else}
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default_watermark')}" {if $jqZoomEnabled}class="jqzoom" alt="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default_watermark')}"{else} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" {/if} id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}" />
			{/if}
		{else}
			...
			<ul id="thumbs_list_frame">
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
						...
						{else}
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, thickbox_default_watermark)}" rel="other-views" class="thickbox {if $smarty.foreach.thumbnails.first}shown{/if}" title="{$image.legend|htmlspecialchars}">
							<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default_watermark')}" alt="{$image.legend|htmlspecialchars}" height="{$mediumSize.height}" width="{$mediumSize.width}" />
						</a>
						{/if}
					</li>
					{/foreach}
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				...
				<input type="text" name="qty" id="quantity_wanted" class="text" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" size="3" maxlength="5" {if $product->minimal_quantity > 1}onkeyup="checkMinimalQuantity({$product->minimal_quantity});"{/if} />
			</p>
						...
						{if $quantity_discounts}
							{foreach from=$quantity_discounts item='quantity_discount' name='quantity_discounts'}
								...
								{else}
										...
										{$newPrice = $productPrice-round($disPrice,1)|floatval}
										{$price=$newPrice|floatval}
								{/if}
								{if !isset($low)} {$low = $price}
								{elseif $low > $price}{$low = $price}
								{/if}
							{/foreach}
						<span id="our_price_display">
							{convertPrice price=$low|floatval} - {convertPrice price=$productPrice}
						</span><br>
						{else}
							<span id="our_price_display">{convertPrice price=$productPrice}</span><br />
						{/if}
					{*Discount patch *}
				{/if}
				</p>
...
<!-- 				<br class="clear" /> -->
			...
			{if isset($stock) && ($stock == "Slut i lager" || $stock == "Out of stock")}
...
<!-- 						<strong style="font-size:14px;color:#ff0000;">{$stock|escape:'htmlall':'UTF-8'}</strong><br> -->
			...
			{/if}
				...
				<div style="width:300px;float:right;font-size:14px;margin-top:50px;">
				<strong>{l s='Quantity discount'}</strong>
				<hr>
					<style type="text/css">
						th.padded, td.padded {
							padding-bottom:5px;
					...
					<table>
						<thead style="text-transform: lowercase;">
							<tr>
								<th style="width:90px;font-weight:bold;" class="padded">{l s='from (qty)'}</th>
								<th style="width:70px;font-weight:bold;" class="padded">Pris</th>
							</tr>
						</thead>
						<tbody>
							{foreach from=$quantity_discounts item='quantity_discount' name='quantity_discounts'}
...
<!-- Disabel orginal discount table
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
