...
<form id="productsSortForm" action="{$request|escape:'htmlall':'UTF-8'}">
	<p class="select">
		...
		<select id="selectPrductSort" class="selectProductSort">
			...
			{/if}
				<option value="reference:asc" {if $orderby eq 'reference' AND $orderway eq 'asc'}selected="selected"{/if}>Referens: Stigande</option>
				<option value="reference:desc" {if $orderby eq 'reference' AND $orderway eq 'desc'}selected="selected"{/if}>Referens: Fallande</option>
		</select>
	</p>
</form>
<!-- /Sort products -->
