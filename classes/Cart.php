...
class CartCore extends ObjectModel
{
		...
		foreach ($package_list as $id_address => $packages)
		{
			...
			foreach ($packages as $id_package => $package)
			{
				...
				foreach ($package['carrier_list'] as $id_carrier)
				{
					...
					$price_with_tax = $this->getPackageShippingCost($id_carrier, true, $country, $package['product_list'],$package['no_shipping_cost']);
					$price_without_tax = $this->getPackageShippingCost($id_carrier, false, $country, $package['product_list'],$package['no_shipping_cost']);
	...
	*/
	 ...
	 * @return float Shipping total
	 */
	public function getPackageShippingCost($id_carrier = null, $use_tax = true, Country $default_country = null, $product_list = null, $no_shipping_cost = null)
	{
		...
		// Check if carrier is in range
		if ($id_carrier && !$this->isCarrierInRange((int)$id_carrier, (int)$id_zone))
			$id_carrier = '';
		// if no carrier and default carrier is in range
		...
		if (empty($id_carrier))
		{
			// Get customer id and load carriers for customer group
			...
			// loop the returned carriers
			foreach ($result as $k => $row)
			{
				// uhm? if carrier is default, go on, and if not default carrier go on.
				if ($row['id_carrier'] == Configuration::get('PS_CARRIER_DEFAULT'))
					continue;
				// make sure it set in the car $carrier
				...
				{
					// check if carrier has a weight within range
					$check_delivery_price_by_weight = Carrier::checkDeliveryPriceByWeight($row['id_carrier'], $this->getTotalWeight(), (int)$id_zone);

					// check if carrier has a price within range
					$total_order = $this->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING, $product_list);
					$check_delivery_price_by_price = Carrier::checkDeliveryPriceByPrice($row['id_carrier'], $total_order, (int)$id_zone, (int)$this->id_currency);

					// THE BUG?
					// There is nothing here that tells $carrier that it should remove the carrier
